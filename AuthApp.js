...
auth_app.get("/", function (req, res) {
    db.getApps(req.logged_user.id,apps => {
        res.render("app", { 'apps': apps });
    });
});
auth_app.post("/", function (req, res) {
    ...
    if (!app_details.id) {
        db.createApp(app_details, response => {
            res.redirect("/app");
        });
    } else {
        db.updateApp(app_details, response => {
            res.redirect("/app");
        });
    }
});
auth_app.post("/delete", function (req, res) {
    ...
    app_details.owner = req.logged_user.id;
    if (app_details.id) {
        db.deleteApp(app_details, response => {
            res.redirect("/app");
        });
    } else {
        res.redirect("/app");
